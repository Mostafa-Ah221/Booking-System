...
export default function Recruiters() {
    ...
    const { recruiters, loading } = useSelector((state) => state.recruiters);
    ...
    const canAddUser = !expired && planKey === "premium";
    ...
    const handleClose = (shouldRefresh = false) => {
        setOpenForm(null);
        if (shouldRefresh) dispatch(getRecruiters());
    };
        ...
        return (
            <div className="flex justify-between items-center mt-4 px-6 py-3 bg-white border-t">
                <div className="text-sm text-gray-600">
                    Showing {indexOfFirstRecruiter + 1} to{' '}
                    {Math.min(indexOfLastRecruiter, filteredRecruiters.length)} of{' '}
                    {filteredRecruiters.length} Users
                </div>
            ...
            <div className="flex items-center justify-end gap-3 mb-5">
                ...
                {canCreateStaff && (
                    canAddUser ? (
                        ...
                        <button
                            className="flex items-center gap-2 bg-gray-200 text-gray-400 px-4 py-3 rounded-md cursor-not-allowed text-sm font-medium"
                            title="Upgrade to Premium to add users"
                            disabled
                        >
                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-gray-50">
                            <tr>
                                <th className="text-center px-6 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider">Parent</th>
                                <th className="text-center px-6 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider">Name</th>
                                <th className="text-center px-6 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider">Email</th>
                                <th className="text-center px-6 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider">Phone</th>
                            ...
                            {currentRecruiters.length === 0 ? (
                                ...
                                currentRecruiters.map((recruiter) => (
                                    <tr key={recruiter.id} className="hover:bg-gray-50">
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-center max-w-[150px] truncate" title={recruiter?.parent}>
                                            {recruiter?.parent || '-'}
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-left max-w-[150px] truncate" title={recruiter?.name}>
                                            {recruiter?.name || '-'}
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-center max-w-[150px] truncate" title={recruiter?.email}>
                                            {recruiter?.email || '-'}
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-center max-w-[150px] truncate" title={recruiter?.phone}>
                                            {recruiter?.phone || '-'}
                                        </td>
