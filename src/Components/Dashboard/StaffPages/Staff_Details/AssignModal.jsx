...
import {   assignInterViewToStaff, fetchAllInterviews } from '../../../../redux/apiCalls/interviewCallApi';
import { useDispatch, useSelector } from 'react-redux';
import { useParams } from 'react-router-dom';
import { assignWorkSpToStaff, getAllWorkspaces } from '../../../../redux/apiCalls/workspaceCallApi';

const AssignModal = ({ isOpen, onClose, activeTab }) => {
  ...
  const { workspaces, workspace,allWorkspaces } = useSelector(state => state.workspace);
  ...
  const getData = () => {
    const data = activeTab === 'workspace' ? allWorkspaces : allInterviews;
  ...
  const handleAssign = async () => {
    ...
    try {
      const formData = {
        [activeTab === 'workspace' ? 'workspace_id' : 'interview_id']: selectedItems
      };
      ...
      if (activeTab === 'workspace') {
        ...
        result = await dispatch(assignInterViewToStaff(id, formData));
      }
                    ...
                    <div className="flex items-center gap-3 flex-1">
                      ...
                      <div className="flex flex-col">
                        ...
                        <span className="text-xs text-gray-500">
                          {activeTab === 'workspace' 
                            ? `${item.description || 'Workspace'} • ${item.members_count || 0} members`
                            : `${item.type} • ${item.duration_cycle} ${item.duration_period}`
                          }
                        </span>
                      </div>
                    </div>
