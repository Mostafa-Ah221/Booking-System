...
const ThemeFooterSection = ({
  ...
  const handleSocialLinksChange = (field, value) => {
    onFooterChange({ ...socialLinks, [field]: value });
  };

  const handlePagePropertiesChange = (field, value) => {
    onPagePropertiesChange({ ...pageProperties, [field]: value });
  };

  const handlePageTextsChange = (field, value) => {
    onPageTextsChange({ ...pageTexts, [field]: value });
  };
      ...
      <div>
        ...
        {openSection === 'pageTexts' && (
          <div className="p-4 bg-gray-50 space-y-4">
            {PAGE_TEXT_FIELDS
            .filter(({ key }) => {
              if (key === 'work_space_text' && userType === 'staff') return false;
              return true;
