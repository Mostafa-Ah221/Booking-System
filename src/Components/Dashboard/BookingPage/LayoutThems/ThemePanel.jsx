...
const ThemePanel = ({
  ...
  useEffect(() => {
    if (theme) {
      ...
      if (uiLayout) { setSelectedLayout(uiLayout); onLayoutChange(uiLayout); }
      ...
      if (parsedColors.primary) {
        ...
        if (matchingColor) {
          setSelectedColor(matchingColor); onColorChange(matchingColor);
        } else {
          ...
          setCustomColor(cust); setSelectedColor(cust); onColorChange(cust);
        }
      }
// في ThemePanel.jsx useEffect
console.log('remove_brand:', theme.remove_brand);
      ...
      onFooterChange({
        facebook:  theme.footer_facebook  || "", visibleFacebook:  toBool(theme.show_facebook),
        instagram: theme.footer_instagram || "", visibleInstagram: toBool(theme.show_instagram),
        x:         theme.footer_x         || "", visibleX:         toBool(theme.show_x),
        linkedin:  theme.footer_linkedin  || "", visibleLinkedin:  toBool(theme.show_linkedin),
        tiktok:    theme.footer_tiktok    || "", visibleTiktok:    toBool(theme.show_tiktok),
        snapchat:  theme.footer_snapchat  || "", visibleSnapchat:  toBool(theme.show_snapchat),
        whatsapp:  theme.footer_whatsapp  || "", visibleWhatsapp:  toBool(theme.show_whatsapp),
        phone:     theme.footer_phone     || "", visiblePhone:     toBool(theme.show_phone),
        email:     theme.footer_email     || "", visibleEmail:     toBool(theme.show_email),
        removeBrand: toBool(theme.remove_brand),
      });
    }
  }, [theme]);
  ...
  const handleLayoutChange = (layout) => { setSelectedLayout(layout); onLayoutChange(layout); };
  ...
  return (
    <>
      ...
      {/* ✅ onClickCapture يشتغل قبل أي handler جوّاه */}
      <div
        className="w-full max-w-md bg-white rounded-lg shadow pb-3"
        onClickCapture={handleSaveInterceptor}
      >
        <div className="divide-y">
                    ...
                    {colorOptions.map((color, index) => (
                      <button
                        ...
                        className={`w-10 h-10 rounded-full overflow-hidden border-2 transition-all ${
                          selectedColor?.color1 === color.color1 && selectedColor?.color2 === color.color2
          ...
          {/* ===== Footer + Page Properties + Page Texts ===== */}
          <ThemeFooterSection
